...
|**header.cellStyle** <br> `{object}`                     |{...}      |header内`单元格的样式`。`注意`：此style里面的width将失效，因本组件的header单元格宽度自动根据body内单元格宽度进行适配，如要设置每一列单元格宽度，请见                                                                       |
...
|**~~body.cell.iconStyle~~** <br> `{object}`              |{...}      |单元格内的图标样式。这个特性可能在后面版本被移除或用其他方式代替，事实上，目前已经存在替换这种方式的方案，比如`对象单元格`                                                    |
