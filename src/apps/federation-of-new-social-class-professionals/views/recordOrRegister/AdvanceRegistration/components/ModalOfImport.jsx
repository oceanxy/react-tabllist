...
export default Form.create({})({
  ...
  methods: {
    ...
    async importFile(info) {
      ...
      const { status, data } = await apis.foretellImportFile(formData)
    ...
    onSubmit() {
      verificationDialog(async () => {
        const status = await apis.foretellImportSuccessData()
    ...
    async downloadErrorData() {
      const { status } = await this.$store.dispatch('export', {
        ...
        fileName: '预告登记失败数据',
        customApiName: 'foretellDownFailExcel'
      })
    ...
    async downloadTemplate() {
      const { status, data } = await apis.foretellGetTemplateUrl()
      const dataUrl = data + '?attname=预告登记导入模板.xlsx'

      if (status) {
        this.download(dataUrl, '预告登记导入模板')
      }
                ...
                this.importSucceeded ? (
                  <Spin spinning={this.tablLoading}>
                    <h3>
                      ...
                      <Button
                        disabled={!this.tableData.failSize > 0}
