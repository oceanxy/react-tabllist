...
export default Form.create({})({
  ...
  methods: {
    ...
    async importFile(info) {
      ...
      const { status, data } = await apis.netSignImportFile(formData)
    ...
    onSubmit() {
      verificationDialog(async () => {
        const status = await apis.netSignImportSuccessData()
    ...
    async downloadErrorData() {
      const { status } = await this.$store.dispatch('export', {
        ...
        fileName: '网签失败数据',
        customApiName: 'netSignDownFailExcel'
      })
    ...
    async downloadTemplate() {
      const { status, data } = await apis.netSignGetTemplateUrl()
      const dataUrl = data + '?attname=网签预告导入模板.xlsx'

      if (status) {
        this.download(dataUrl, '网签预告导入模板')
      }
